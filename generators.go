...
func GenerateRequestHeaders(parameters openapi3.Parameters, valueMaker ValueMaker) any {
    ...
    for _, paramRef := range parameters {
        ...
        for paramName, schemaRef := range schema.Properties {
            state := &GeneratorState{NamePath: []string{paramName}, IsHeader: true}
...
func GenerateResponseHeaders(headers openapi3.Headers, valueMaker ValueMaker) map[string]any {
    res := map[string]any{}

    for name, headerRef := range headers {
        state := &GeneratorState{NamePath: []string{name}, IsHeader: true}
