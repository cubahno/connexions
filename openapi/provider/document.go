...
func (op *KinOperation) GetResponse() *openapi.Response {
	...
	for name, header := range libResponse.Headers {
		...
		headers[name] = &openapi.Parameter{
			Name:     name,
...
func newSchemaFromKin(schema *openapi3.Schema, parseConfig *config.ParseConfig, refPath []string, namePath []string) *openapi.Schema {
	if schema == nil {
		return nil
	}
...
func mergeKinSubSchemas(schema *openapi3.Schema) (*openapi3.Schema, string) {
	...
	for _, schemaRef := range allOf {
		...
		if impliedType == openapi.TypeArray && subSchema.Items != nil && subSchema.Items.Value != nil {
			if subRef == "" {
