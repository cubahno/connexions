...
func ApplySchemaConstraints(openAPISchema any, res any) any {
	...
	schema, ok := openAPISchema.(*openapi.Schema)
	if !ok || schema == nil {
		return res
	}
...
func applySchemaStringConstraints(schema *openapi.Schema, value string) any {
	if schema == nil {
		return value
	}
	...
	if pattern != "" && !internal.ValidateStringWithPattern(value, pattern) {
		...
		value = createStringFromPattern(pattern, int(maxLength))
    ...
    // TODO: skip if generated by pattern
...
func applySchemaNumberConstraints(schema *openapi.Schema, value float64) float64 {
	if schema == nil {
		return value
	}
...
func createStringFromPattern(pattern string, length int) string {
	if pattern == "" {
		return ""
	}
    if length == 0 {
	...
	result, err := reggen.Generate(pattern, length)
	if err != nil {
		return ""
	}
	return result
