...
func newOpenAPIParameter(name, in string, schema *types.Schema) *Parameter {
	return &Parameter{
		Name:   name,
		In:     in,
		Schema: schema,
...
func TestNewRequestFromOperation(t *testing.T) {
	...
	t.Run("base-case", func(t *testing.T) {
		valueReplacer := func(content any, state *replacer.ReplaceState) any {
			schema, _ := content.(*types.Schema)
	...
	t.Run("invalid-resolve-value", func(t *testing.T) {
		valueReplacer := func(content any, state *replacer.ReplaceState) any { return func() {} }
		operation := &KinOperation{Operation: openapi3.NewOperation()}
...
func TestGenerateURLFromSchemaParameters(t *testing.T) {
	...
	t.Run("replaced-with-empty-dont-happen", func(t *testing.T) {
		path := "/users/{id}/{file-id}"
		valueResolver := func(content any, state *replacer.ReplaceState) any { return "" }
...
func TestGenerateContentFromSchema(t *testing.T) {
	...
	t.Run("fast-track-resolve-null-string", func(t *testing.T) {
		...
		res := GenerateContentFromSchema(schema, valueResolver, replacer.NewReplaceStateWithName("name"))
		assert.Nil(res)
	})
