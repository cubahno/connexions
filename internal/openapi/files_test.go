...
func TestGetPropertiesFromOpenAPIFile(t *testing.T) {
	...
	t.Run("nested-with-index-name", func(t *testing.T) {
		...
		err := types.SaveFile(filePath, contents)
		assert.NoError(err)
...
func TestCopyDirectory(t *testing.T) {
	...
	t.Run("happy-path", func(t *testing.T) {
		...
		base2 := t.TempDir()
		err := types.CopyDirectory(base1, base2)
		assert.Nil(err)
	...
	t.Run("invalid-source", func(t *testing.T) {
		err := types.CopyDirectory("/non-existent", "/")
		assert.Error(err)
	})
}
...
func TestCleanupServiceFileStructure(t *testing.T) {
	...
	t.Run("happy-path", func(t *testing.T) {
		...
		err = types.CleanupServiceFileStructure(tempDir)
		if err != nil {
	...
	t.Run("file-insteadof-dir", func(t *testing.T) {
		...
		_ = types.SaveFile(f, []byte("test"))

		err := types.CleanupServiceFileStructure(f)
		assert.Nil(err)
	})
}
...
func TestIsEmptyDir(t *testing.T) {
	assert := assert2.New(t)
	tempDir := t.TempDir()
	assert.True(types.IsEmptyDir(tempDir))
	...
	_ = file.Close()
	assert.False(types.IsEmptyDir(tempDir))
}

func TestIsJsonType(t *testing.T) {
	assert := assert2.New(t)
	assert.True(types.IsJsonType([]byte(`{"key": "value"}`)))
	assert.False(types.IsJsonType([]byte(`foo: bar`)))
}

func TestIsYamlType(t *testing.T) {
	assert := assert2.New(t)
	assert.True(types.IsYamlType([]byte(`foo: bar`)))
	assert.False(types.IsYamlType([]byte(`100`)))
}
...
func TestExtractZip(t *testing.T) {
	...
	t.Run("happy-path", func(t *testing.T) {
		...
		targetDir := t.TempDir()
		err = types.ExtractZip(&zipReader.Reader, targetDir, []string{"take-this", "take-too"})
		if err != nil {
	...
	t.Run("invalid-dest", func(t *testing.T) {
		...
		res := types.ExtractZip(&zipReader.Reader, dest, []string{"take-this", "take-too"})
		assert.Error(res)
	})
}
...
func TestGetFileContentsFromURL(t *testing.T) {
	...
	t.Run("invalid-url", func(t *testing.T) {
		...
		_, _, err := types.GetFileContentsFromURL(nil, "unknown-url")
		assert.Error(err)
	})
	...
	t.Run("status-error", func(t *testing.T) {
		...
		_, _, err := types.GetFileContentsFromURL(nil, mockServer.URL)
	...
	t.Run("read-error", func(t *testing.T) {
		...
		_, _, err := types.GetFileContentsFromURL(client, mockServerWithReadError.URL)
		if err == nil {
	...
	t.Run("happy-path", func(t *testing.T) {
		...
		content, contentType, err := types.GetFileContentsFromURL(nil, mockServer.URL)
		assert.NoError(err)
