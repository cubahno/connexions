...
func TestValidator_ValidateRequest_NonJSON(t *testing.T) {
	doc, err := NewDocumentFromFile(filepath.Join(testDataPath, "document-connexions.yml"))
	assert.Nil(t, err)
	validator := NewValidator(doc)
...
func TestValidator_ValidateResponse_NonJSON(t *testing.T) {
	doc, err := NewDocumentFromFile(filepath.Join(testDataPath, "document-with-other-responses.yml"))
	assert.Nil(t, err)
	validator := NewValidator(doc)
...
func TestValidator_ValidateResponse_NoSchema(t *testing.T) {
	doc, err := NewDocumentFromFile(filepath.Join(testDataPath, "document-without-response.yml"))
	assert.Nil(t, err)
	validator := NewValidator(doc)
