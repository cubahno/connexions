...
func HasCorrectSchemaValue(ctx *ReplaceContext, value any) bool {
	...
	schema, ok := ctx.Schema.(*types.Schema)
	if !ok || schema == nil {
		return true
	}
...
func ApplySchemaConstraints(openAPISchema any, res any) any {
	...
	schema, ok := openAPISchema.(*types.Schema)
	if !ok || schema == nil {
		return res
	}
...
func applySchemaStringConstraints(schema *types.Schema, value string) any {
	if schema == nil {
		return value
	}
...
func applySchemaNumberConstraints(schema *types.Schema, value float64) float64 {
	if schema == nil {
		return value
	}
...
func IsMatchSchemaReadWriteToState(schema *types.Schema, state *ReplaceState) bool {
	// unable to determine
