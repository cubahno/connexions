...
func newLazyRegistry(openapiSpec []byte, cfg codegen.Configuration, specOptions *config.SpecOptions) typedef.OperationRegistry {
	start := time.Now()
	slog.Info("Creating lazy registry", "service", "petstore")
	...
	slog.Info("Lazy registry created", "service", "petstore", "duration", time.Since(start), "routes", len(registry.GetRouteInfo()))
	return registry
}
...
func newEagerRegistry(openapiSpec []byte, cfg codegen.Configuration, specOptions *config.SpecOptions) typedef.OperationRegistry {
	start := time.Now()
	slog.Info("Parsing OpenAPI spec", "service", "petstore")
	...
	slog.Info("OpenAPI spec parsed", "service", "petstore", "duration", time.Since(start), "operations", len(parseCtx.Operations))

	start = time.Now()
	slog.Info("Building typedef registry", "service", "petstore")
	registry := typedef.NewTypeDefinitionRegistry(parseCtx, 0, openapiSpec)
	slog.Info("Typedef registry built", "service", "petstore", "duration", time.Since(start))
