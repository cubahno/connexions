...
import (
	...
	"example/services/petstore/types"
...
func (h *Handler) getOperationData(path, method string) *operationData {
	key := method + ":" + path

	// Check cache first
	if cached, ok := h.opCache.Load(key); ok {
		return cached.(*operationData)
	}

	// Load from registry
	op := h.registry.FindOperation(path, method)
	if op == nil {
		return nil
	}
...
func (h *Handler) Generate(w http.ResponseWriter, r *http.Request) {
	...
	opData := h.getOperationData(req.Path, req.Method)
	var op *schema.Operation
	if opData != nil {
		op = opData.operation
	}
	res := h.generator.Request(&req, op)
	slog.Debug("Generated request", "request", string(res))
	api.NewJSONResponse(w).Send(res)
}

// getError generates an error response for the given operation
func (h *Handler) getError(path, method string, err error) []byte {
	opData := h.getOperationData(path, method)
	if opData != nil && opData.responseSchema.Error != nil {
		opKey := method + ":" + path
		return h.generator.Error(opData.responseSchema.Error, errorPaths[opKey], err.Error())
	}
...
func (h *Handler) addPet(w http.ResponseWriter, r *http.Request) {
	...
	opData := h.getOperationData(opPath, opMethod)
	if opData == nil {
		http.Error(w, "operation not found", http.StatusInternalServerError)
		return
	}

	res := h.generator.Response(opData.responseSchema)
	resValue := res.Body
	statusCode := 200
