...
func newLazyRegistry(openapiSpec []byte, cfg codegen.Configuration, specOptions *config.SpecOptions) typedef.OperationRegistry {
	start := time.Now()
	slog.Info("Creating lazy registry", "service", "static")
	...
	slog.Info("Lazy registry created", "service", "static", "duration", time.Since(start), "routes", len(registry.GetRouteInfo()))
	return registry
}
...
func newEagerRegistry(openapiSpec []byte, cfg codegen.Configuration, specOptions *config.SpecOptions) typedef.OperationRegistry {
	start := time.Now()
	slog.Info("Parsing OpenAPI spec", "service", "static")
	...
	slog.Info("OpenAPI spec parsed", "service", "static", "duration", time.Since(start), "operations", len(parseCtx.Operations))

	start = time.Now()
	slog.Info("Building typedef registry", "service", "static")
	registry := typedef.NewTypeDefinitionRegistry(parseCtx, 0, openapiSpec)
	slog.Info("Typedef registry built", "service", "static", "duration", time.Since(start))
